
<?php $__env->startSection('content'); ?>
    <div class="container-fluid">
        <div class="row justify-content-md-center">
            <div class="about">
                <div class="col-12">
                    <div class="row">
                        <div class="aboutUs">
                            <h3><b>ABOUT PWG GROUP</b></h3>
                            <p>People are the greatest value for our company</p>
                            <p>
                                PWG Group is a leading VISA service provider that helps connect YOU to the WORLD.
                            </p>
                            <p>
                                With 8 years in the business, PWG Group continues to be determined in providing the easiest visa application processes for individuals seeking 
                                to migrate to Europe and Canada;  providing them assistance, assurance and updates along their journey.  We have a high success rate 
                                of over 4000 VISAS (and counting) issued to our customers, a start of their dream to live and work in Europe and Canada.
                                We extend our services to applicants and clients from various sectors – from chefs and engineers to management of business processes or 
                                specialized services to the industry. 
                            </p>
                            <button class="btn learn">Learn More</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row justify-content-md-center">
            <div class="about-sub">
                <div class="col-12">
                    <div class="row">
                        <div class="col-sm-12 col-md-12 col-lg-6 nopadding">
                            <div class="aboutleft">
                                <div class="row">
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/complex-support.png')); ?>">
                                            <p>
                                                Complex support for acquiring citizenship
                                            </p>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/practical-info.png')); ?>">
                                            <p>
                                                Practical Information on Amenities
                                            </p>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/child-social.png')); ?>">
                                            <p>
                                                Child & Social Welfare Schemes information 
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/relative-support.png')); ?>">
                                            <p>
                                                Relatives Support & Guarantee Their Permanent Residence
                                            </p>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/guidence.png')); ?>">
                                            <p>
                                                Guidance for renting your own property
                                            </p>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-4 col-lg-4">
                                        <div class="aboutus-icon">
                                            <img src="<?php echo e(asset('images/affiliate/assistance.png')); ?>">
                                            <p>
                                                Assistance & advice on opening a bank account
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-6 nopadding">
                            <div class="aboutright">
                                <h3>
                                    <b>What makes us unique?</b>
                                </h3>
                                <h6>
                                    Unleashing Opportunities
                                </h6>
                                <p>
                                    Poland is a destination preferred by many when in search of 
                                    migrating and looking for work abroad. Working in Poland pro-vides 
                                    several benefits not limited to:
                                    Being the sixth largest economy by GDP in the European Union
                                    No language barrier 
                                    – Poland’s job markets are attractive for multilinguals.
                                    Ranks 8th as the best country for female workers
                                    Low cost of living in comparison to other European countries
                                    Great healthcare based on an all-inclusive insurance system.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row justify-content-md-center">
            <div class="client-count">
                <div class="col-12">
                    <div class="row">
                        <div class="count-banner">
                            <div class="row">
                                <div class="col-sm-12 col-md-4 col-lg-4">
                                    <h1><b>4000 +</b></h1>
                                    <p>Visa Granted and counting</p>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-4">
                                    <h1><b>24/7</b></h1>
                                    <p>Customer support system</p>
                                </div>
                                <div class="col-sm-12 col-md-4 col-lg-4">
                                    <h1><b>2000 +</b></h1>
                                    <p>Employees wolrdwide</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('affiliate.layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Shamshera Hamza\pwg_client_portal\resources\views/affiliate/about-us.blade.php ENDPATH**/ ?>