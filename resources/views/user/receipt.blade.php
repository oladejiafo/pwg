...
<style>
  ...
  <body>
        ...
        <div class="row" style="display: block;text-align: center;padding-top:5px;padding-bottom:10px; @if($discounted > 0) height:135px; @else height:90px; @endif ">
            <div class="col-lg-4" valign="top" style="display: inline-block; float:left; text-align:left">
                BILL TO: <br>
                {{Auth::user()->name . ' ' . Auth::user()->middle_name . ' ' . Auth::user()->sur_name}}
            </div>
            <div class="col-lg-8" align="center" style="display: inline-block;float:right;width:60%;margin:0px">
                <div class="row" style="display: block">
                   <b>
                    <div class="col-lg-4" style="width:30%;display: inline-block; background-color:#eee;height:40px; opacity: 0.7">Payment Date <br>{{date("d-m-Y", strtotime($user->payment_date))}}</div>
                    <div class="col-lg-4" style="width:30%;display: inline-block; background-color:#eee;height:40px; opacity: 0.7">Paid Amount <br>{{number_format($user->paid_amount,2)}}</div>
                    <div class="col-lg-4" style="width:30%;display: inline-block; background-color:#eee;height:40px; opacity: 0.7">Total Due <br>{{number_format($user->invoice_amount,2)}}</div>
                   </b>
                </div>
                @if(Auth::user()->country_of_residence == "United Arab Emirates")
                    ...
                    @php $thisAmt = ($user->paid_amount*(100/105)); $thisVat = $user->paid_amount - $thisAmt; @endphp
                    <div class="row" style="display: block;">
                    <b>
...
@endphp
        ...
        <div class="row" style="display: block">
            ...
            <div align="left" class="col-4" style="width:30%;display: inline-block; height:20px"> First Payment @if($apply->first_payment_status =='Paid') <span style="font-weigth:bold;color:#ccc">[PAID]</span> @endif</div>
